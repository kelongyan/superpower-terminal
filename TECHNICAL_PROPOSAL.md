...
| **渲染** | wgpu + fontdue atlas + DirectWrite 光栅化/度量 | 已完成基础渲染管线，并已扩大 DirectWrite 覆盖 |
...
**当前不足：**
- DirectWrite 仍未完全接管所有 fallback 链
- atlas 虽已推进到 shelf 分配，但仍需继续评估长期运行退化
...
| DirectWrite 当前职责 | 更多字体场景的字形光栅化 + 度量 | 渐进替换 fontdue 主路径 |
