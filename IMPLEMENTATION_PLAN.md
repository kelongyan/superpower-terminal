...
当前项目已经不是“只有最小闭环的 demo”，而是一个**可运行、可试用的 Windows 本地终端雏形**：
...
- `DirectWrite` 目前已完成“可用性探测 + 度量接入”，但尚未替换主字形光栅化链路

**当前判断**：

- 对于 `cmd.exe` / PowerShell 下的基础命令输入、输出、滚动、复制粘贴，已经具备试用能力
- 对于 `vim` / `less` / `fzf` / `tmux` 这类依赖全屏模式、鼠标报告和更完整控制序列的 TUI 工作流，**还没有达到稳定日常使用**
...
下面这些项不是“锦上添花”，而是**从试用走向稳定日常使用**的关键缺口：

### 1. 全屏 TUI 兼容性
- alternate screen buffer（`vim` / `less` / `fzf` / `htop` / `git log --patch` 等都会明显受影响）
- bracketed paste
- 鼠标报告模式
- 更完整的 DEC 私有模式与 CSI 覆盖，减少 `Unhandled CSI` 场景
...
**验收**：具备稳定输入、滚动、选择、复制粘贴、基础着色与缩放能力，并可较稳定运行常见全屏 TUI。
...
因此，后续开发应默认从 **Phase A.2（补齐可正常使用终端所需能力）** 开始，而不是继续平均铺开所有功能点。
